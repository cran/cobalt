...
This is an introduction to the use of `cobalt` with longitudinal treatments. These occur when there are multiple treatment periods spaced over time, with the potential for time-dependant confounding to occur. A common way to estimate treatment effects in these scenarios is to use marginal structural models (MSM), weighted by balancing weights. The goal of applying weights is to simulate a sequential randomization design, where the probability of being assigned to treatment at each time point is independent of each unit's prior covariate and treatment history. For introduction to MSMs in general, see Thoemmes & Ong (2016), VanderWeele, Jackson, & Li (2016), Cole & Hernán (2008), or Robins, Hernán, & Brumback (2000). The key issue addresed by this guide and `cobalt` in general is assessing balance before each treatment period to ensure the removal of confounding.
...
This output will appear no matter what the treatment types are (i.e., binary, continuous, multinomial), but for multinomial treatments or when the treatment types vary, no balance summary will be computed or displayed.

We can use `bal.tab()` with the `weightitMSM` object generated above. Setting `un = TRUE` would produce balance statistic before adjustment, like we did before.

```{r}
bal.tab(Wmsm, un = TRUE)
```

We can use `which.time` and `msm.summary` to focus in on specific time points. `which.time` controls which time periods are displayed; one or more time periods can be entered. `NA` will not display any time periods (so only the summary will be displayed), and `NULL`, the default, will display all time periods. Setting `msm.summary` to `FALSE` will hide the summary across time points.
...
`love.plot()` works with longitudinal treatments just as it does with point treatments, except that the user can choose whether to display seperate plots for each time period or one plot with the summary across time periods. As with `bal.tab()`, the user can set `which.time` to display only certain time periods. When set to `NA`, the summary across time points is displayed. The `agg.fun` argument is automatically set to `"max"` and cannot be changed.
...
Here we used `WeightIt` to generate our MSM weights, but `cobalt` is compatible with other packages for longitudinal treatments as well. `CBMSM` objects from the `CBPS` package and `iptw` objects from the `twang` package can be used in place of the `weightitMSM` object in the above examples. In addition, users who have generated balancing weights outside any of these package can simply specify an argument to `weights` in `bal.tab()` with the formula or data frame methods to assess balance using those weights.

Note that `CBPS` estimates and assess balance on MSM weights differently than `twang` and `cobalt`. Its focus is on ensuring balance across all treatment history permutations, whereas `cobalt` focuses on evaluating the possibility of sequential randomization. For this reason, it may appear that `CBMSM` objects have different balance qualities as measured by the two packages.

##References
Cole, S. R., & Hernan, M. Á. (2008). Constructing Inverse Probability Weights for Marginal Structural Models. American Journal of Epidemiology, 168(6), 656–664. https://doi.org/10.1093/aje/kwn164
